...
class DINA(nn.Module):
    ...
    def forward(self, contexts):
        ...
        batch_sz = M.shape[0]
        M = M.repeat(self.heads, 1, 1, 1)
        U = [self.U[i, :, :].squeeze().repeat(batch_sz, 1, 1) for i in range(self.heads)]
        U = torch.stack(U)
        m_shape = M.shape
        M_3d = M.reshape(self.heads, -1, M.shape[-1])
        U_3d = U.reshape(self.heads, M.shape[-1], -1)
        K = M_3d.bmm(U_3d).bmm(M.view(m_shape[0], -1, m_shape[2]))
        # normalize each sample
        # K = self.batch_norm_2d(K.unsqueeze(dim=1)).squeeze()
        K = K * scaling
        K = self.activation(K)
        K = K.reshape(self.heads, batch_sz, K.shape[-1], K.shape[-1])
        ...
        for i, context in enumerate(contexts):
            ...
            # C = context.repeat(self.heads, 1, 1, 1)
