...
class CPIBaseline(Trainer):
    ...
    @staticmethod
    def evaluate(eval_dict, y, y_pred, w, metrics):
        y = y.reshape(-1, )
        w = w.reshape(-1, )
        y_pred = torch.max(y_pred, dim=1)[1].cpu().detach().numpy()
        for metric in metrics:
            eval_dict[metric.__name__] = metric(y, y_pred, sample_weight=w)
        return np.mean(eval_dict['f1_score'])
    ...
    def train(model, optimizer, data_loaders, metrics, frozen_models, transformers_dict, prot_desc_dict, tasks,
        ...
        e_avg = ExpAverage(.01)
        print('len;', len(data_loaders["train"]))
...
def get_hparam_config(flags, view):
    return {
        ...
        # "hdims": DiscreteParam(min=32, max=1024, size=DiscreteParam(min=1, max=2)),
        "output_dim": ConstantParam(2),
        ...
        "tr_batch_size": CategoricalParam([1, 16, 32, 128]),
